...
import 'package:yajid/widgets/environment_banner.dart';
...
void main() async {
  ...
  // Configure Crashlytics (disabled in dev environment)
  if (currentConfig.enableCrashlytics) {
  ...
  // Enable Firebase Performance Monitoring (disabled in dev)
    ...
    return BlocBuilder<AuthBloc, AuthState>(
      builder: (context, state) {
        if (currentConfig.enableLogging) {
          print('AuthWrapper [${currentConfig.environmentName}]: Current auth state: ${state.runtimeType}');
        }

        if (state is AuthInitial || state is AuthLoading) {
          if (currentConfig.enableLogging) {
            print('AuthWrapper [${currentConfig.environmentName}]: Showing loading screen');
          }
        ...
        if (state is AuthAuthenticated) {
          if (currentConfig.enableLogging) {
            print('AuthWrapper [${currentConfig.environmentName}]: User authenticated, userId: ${state.user.uid}');
          }
        ...
        if (state is AuthError) {
          if (currentConfig.enableLogging) {
            print('AuthWrapper [${currentConfig.environmentName}]: Auth error: ${state.message}');
          }
        }

        // User is unauthenticated, clear onboarding provider after build
        if (currentConfig.enableLogging) {
          print('AuthWrapper [${currentConfig.environmentName}]: User unauthenticated, showing auth screen');
        }
        WidgetsBinding.instance.addPostFrameCallback((_) {
...
class _OnboardingWrapperState extends State<OnboardingWrapper> {
  ...
  void initState() {
    super.initState();
    if (currentConfig.enableLogging) {
      print('OnboardingWrapper [${currentConfig.environmentName}]: initState for userId: ${widget.userId}');
    }
    // Load onboarding status after the first frame
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!_hasLoadedOnboarding) {
        if (currentConfig.enableLogging) {
          print('OnboardingWrapper [${currentConfig.environmentName}]: Loading onboarding status in postFrameCallback');
        }
        final onboardingProvider = Provider.of<OnboardingProvider>(context, listen: false);
  ...
  void didUpdateWidget(OnboardingWrapper oldWidget) {
    ...
    if (currentConfig.enableLogging) {
      print('OnboardingWrapper [${currentConfig.environmentName}]: didUpdateWidget, old: ${oldWidget.userId}, new: ${widget.userId}');
    }
    // If user changed, reload onboarding status
    if (oldWidget.userId != widget.userId) {
      if (currentConfig.enableLogging) {
        print('OnboardingWrapper [${currentConfig.environmentName}]: User changed, reloading onboarding status');
      }
  ...
  Widget build(BuildContext context) {
    return Consumer<OnboardingProvider>(
      builder: (context, onboardingProvider, child) {
        if (currentConfig.enableLogging) {
          print('OnboardingWrapper [${currentConfig.environmentName}]: Building, isLoading: ${onboardingProvider.isLoading}, isCompleted: ${onboardingProvider.isOnboardingCompleted}');
        }

        if (onboardingProvider.isLoading) {
          if (currentConfig.enableLogging) {
            print('OnboardingWrapper [${currentConfig.environmentName}]: Showing loading screen');
          }
          return const Scaffold(
            body: Center(
              child: CircularProgressIndicator(),
        ...
        if (!onboardingProvider.isOnboardingCompleted) {
          if (currentConfig.enableLogging) {
            print('OnboardingWrapper [${currentConfig.environmentName}]: Showing onboarding screen');
          }
          return const OnboardingScreen();
        }

        if (currentConfig.enableLogging) {
          print('OnboardingWrapper [${currentConfig.environmentName}]: Showing main navigation screen');
        }
